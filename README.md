# Soustack (Svelte + Vite)

This repository now hosts the Soustack recipe collection built with **Svelte + Vite**. The app keeps the original JSON recipe format, adds component-based UI, client-side state stores, ingredient scaling, mise-en-place mode, Google Tasks shopping-list export, and continues to work as a static Progressive Web App deployable to both Vercel and GitHub Pages.
